...
export default function Gallery() {
  ...
  const extractPartNumber = (img: any): string => {
    // Try to get part number from various fields
    ...
    if (img.notes) {
      const partMatch = img.notes.match(/Part:\s*([A-Za-z0-9\-_]+)/i) ||
  ...
  const groupImagesByPartNumber = (images: ImageType[]): GroupedImages[] => {
    ...
    Object.values(grouped).forEach(group => {
      ...
      group.images.sort((a, b) => new Date(b.captured_at).getTime() - new Date(a.captured_at).getTime());
      
      // Get latest upload date
      group.latestUpload = group.images[0]?.uploadDate || 'Unknown';
      
      // Calculate total size
      const totalBytes = group.images.reduce((sum, img) => sum + (img.image_size || 0), 0);
      group.totalSize = totalBytes > 0 ? `${(totalBytes / (1024 * 1024)).toFixed(1)} MB` : 'Unknown';
    });

    return Object.values(grouped).sort((a, b) => a.partNumber.localeCompare(b.partNumber));
  };
  ...
  const filteredGroupedImages = groupedImages.filter(group =>
    group.partNumber.toLowerCase().includes(searchQuery.toLowerCase()) ||
    (group.partName && group.partName.toLowerCase().includes(searchQuery.toLowerCase()))
  );

  // Get selected group details
  const selectedGroup = selectedPartNumber 
    ? filteredGroupedImages.find(group => group.partNumber === selectedPartNumber)
    : null;

  // Calculate totals
  const totalParts = filteredGroupedImages.length;
  const totalImages = filteredGroupedImages.reduce((sum, group) => sum + group.totalImages, 0);
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <div className="bg-gray-100 rounded-lg flex items-center justify-center min-h-[300px]">
                          ...
                          <div className="hidden text-center">
                            ...
                            <div className="text-sm text-gray-400">{selectedImage.name}</div>
                          </div>
                        </div>
                          ...
                          <div className="bg-gray-50 p-4 rounded-lg">
                            ...
                            <div className="grid grid-cols-1 gap-3 text-sm">
                              <div className="flex justify-between">
                                ...
                                <span className="font-medium">{selectedImage.resolution || '1920x1080'}</span>
                              </div>
                              
                              <div className="flex justify-between">
                                <span className="text-gray-500">Camera:</span>
                                <span className="font-medium">{selectedImage.device_model || 'Unknown'}</span>
                              </div>
                              
                              <div className="flex justify-between">
                                <span className="text-gray-500">Location:</span>
                                <span className="font-medium">{selectedImage.location || 'Unknown'}</span>
                              </div>
                              
                              {selectedImage.capture_mode && (
                                <div className="flex justify-between">
                                  <span className="text-gray-500">Capture Mode:</span>
                                  <span className="font-medium">{selectedImage.capture_mode}</span>
                                </div>
                              )}
                            </div>
                          </div>
