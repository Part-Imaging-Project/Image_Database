// src/app/components/DashboardUI.tsx - Fixed for MinIO folder structure
"use client";
...
const extractPartNumber = (image: ImageData): string => {
  // Priority order: part_number -> part_name -> extract from file_path -> fallback
...
const getDisplayPartNumber = (image: ImageData): string => {
  return extractPartNumber(image);
};
...
const hasValidPartNumber = (image: ImageData): boolean => {
  ...
  return (
    !partNumber.startsWith("PART-") ||
    !!(
      image.part_number ||
      image.part_name ||
      (image.notes &&
        (image.notes.includes("Part:") ||
          image.notes.includes("P/N:") ||
          image.notes.includes("PN:")))
    )
  );
};
...
// Images Table Component - Fixed for folder structure
          ...
          <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
            <div>
              <p className="text-sm text-gray-700">
                ...
                <span className="font-medium">{tabImages.length}</span> of{" "}
